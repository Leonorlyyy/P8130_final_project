...
The figure \@ref{survival_months_by_status} distributed the survival time by the status, showing that survival months are significantly higher for the Alive group compared to the Dead group. According the figure \@ref{tumor_sizes_by_t_stage}, from T1 to T3, as the stage changes, both the mean tumor sizes and IQR become larger. At T4 stage, the IQR of tumor sizes is much larger than others, and the mean size is smaller than the mean size at T3 stage. We notice that there are some potential outliers both ar T1 stage and T3 stage. The survival time is longer in the Regional stage, and the Alive group shows higher survival times across both stages by looking at the figure \@ref{survival_months_by_a_stage_based_on_status}. In the figure \@ref{tumor_size_by_differentiate}, the Undifferentiated group has larger tumor sizes compared to the other categories, while the Well, Moderately, and Poorly differentiated groups show similar distributions with many smaller tumors and numerous high-value outliers. The figure \@ref{relationship_between_age_and_tumor_size_across_status} highlights the differences in tumor size distribution and trends with age between individuals who are alive and those who are deceased. While the "Alive" group shows no significant relationship between age and tumor size, the "Dead" group exhibits a pattern where larger tumors are associated with younger ages. According the figure \@ref{positive_reginol_node_vs_survival_months_across_differentiate}, as it changes from undifferentiated to well differentiated, the negative correlation between the number of positive reginol nodes and the survival months becomes weaker.
...
```{r, echo=FALSE, fig.cap="Relationship Between Age and Tumor Size across Status", include=FALSE}
...
```{r, echo=FALSE, fig.cap="Positive Reginol Node vs Survival Months Across Differentiate", include=FALSE}
